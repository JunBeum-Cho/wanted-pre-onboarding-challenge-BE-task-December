...
- 장점
  1. 비동기(Asynchronous) : 데이터를 수신자에게 바로 보내지 않고 큐에 넣고 관리하기 때문에 나중에 처리 가능
  2. 비동조(Decoupling) : 애플리케이션과 분리할 수 있기 때문에 확장이 용이해짐
  3. 탄력성(Resilience) : 일부가 실패하더라도 전체에 영향을 주지 않음
  4. 과잉(Redundancy) : 실패할 경우 재실행 가능
  5. 보증(Guarantees) : 작업이 처리된 걸 확인할 수 있음
  6. 확장성(Scalable) : N:1:M 구조로 다수의 프로세스들이 큐에 메시지를 보낼 수 있음
- 예시
...
```
AWS의 기초적인 내용과 구조를 구성할 때 어떻게 진행하면 좋을지 배우고 싶습니다.
```
